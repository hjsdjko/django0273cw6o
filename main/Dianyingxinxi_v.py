...
#分类列表
def dianyingxinxi_lists(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":[]}
        msg['data'],_,_,_,_  = dianyingxinxi.page(dianyingxinxi, dianyingxinxi, {})
        return JsonResponse(msg)

def dianyingxinxi_query(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = dianyingxinxi.objects.filter(**request.session.get("req_dict")).values()
...
def dianyingxinxi_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, ROUND(sum({1}),2) total FROM dianyingxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, ROUND(sum({1}),2) total FROM dianyingxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, ROUND(sum({1}),2) total FROM dianyingxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, yColumnName, where, '%Y')
...
def dianyingxinxi_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM dianyingxinxi {2} GROUP BY {0} LIMIT 10".format(xColumnName, yColumnName, where)
        func_name = sys._getframe().f_code.co_name
        table_name = func_name.split('_')[0]
...
def dianyingxinxi_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = ''
            if timeStatType == '日':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, ROUND(sum({1}),2) total FROM dianyingxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, item, where, '%Y-%m-%d')

            if timeStatType == '月':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, ROUND(sum({1}),2) total FROM dianyingxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, item, where, '%Y-%m')

            if timeStatType == '年':
                sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, ROUND(sum({1}),2) total FROM dianyingxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, item, where, '%Y')
...
def dianyingxinxi_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = "SELECT {0}, ROUND(sum({1}),2) AS total FROM dianyingxinxi {2} GROUP BY {0} LIMIT 10".format(xColumnName, item, where)
            L = []
            cursor = connection.cursor()
            cursor.execute(sql)
            desc = cursor.description
...
#批量生成数据
def dianyingxinxi_batch_gen(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        columns=  dianyingxinxi.getallcolumn( dianyingxinxi, dianyingxinxi)
        all_list, _, _, _,_ = dianyingxinxi.page(dianyingxinxi, dianyingxinxi, {})

        batchList=[]
        for i in range(0,int(recordNum)):
            dianyingxinxi_model =dianyingxinxi()
            for column in columns:
                ...
                if column !="id" and column!="addtime" and column!="clicktime" and random_dict[column]!="None":
                    if 'datetime.datetime' in str(type(random_dict[column])):
                        setattr(dianyingxinxi_model, column, random_dict[column].strftime(
                            "%Y-%m-%d %H:%M:%S"))
                    elif 'datetime.date' in str(type(random_dict[column])):
                        setattr(dianyingxinxi_model, column, random_dict[column].strftime("%Y-%m-%d"))
                    elif 'datetime' in str(type(random_dict[column])):
                        setattr(dianyingxinxi_model, column, random_dict[column].strftime("%Y-%m-%d"))
                    else:
                        setattr(dianyingxinxi_model, column, random_dict[column])
            batchList.append(dianyingxinxi_model)
        dianyingxinxi.objects.bulk_create(batchList)
        return JsonResponse(msg)
