...
<div class="content" :style='{"minHeight":"100vh","width":"100%","padding":"30px","background":"url(http://codegen.caihongy.cn/20240406/b4e984e90fb34e1cb7a7aac463fb85da.png) no-repeat center top / cover","height":"auto"}'>
	...
	<div :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('remendianying','首页总数')">
			...
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{remendianyingCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>热门电影总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('dianyingxinxi','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"rgba(44, 55, 66,1)","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-zhangjie18" :style='{"color":"#fff","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{dianyingxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>电影信息总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 5 -->
	<div class="type5" :style='{"alignContent":"flex-start","padding":"10px 20px","borderRadius":"16px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="remendianyingChart1" class="echarts1" v-if="isAuth('remendianying','首页统计')"></div>
		<div id="remendianyingChart2" class="echarts2" v-if="isAuth('remendianying','首页统计')"></div>
		<div id="remendianyingChart3" class="echarts3" v-if="isAuth('remendianying','首页统计')"></div>
		<div id="dianyingxinxiChart1" class="echarts4" v-if="isAuth('dianyingxinxi','首页统计')"></div>
		<div id="dianyingxinxiChart2" class="echarts5" v-if="isAuth('dianyingxinxi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getremendianyingCount();
		this.remendianyingChat1();
		this.remendianyingChat2();
		this.remendianyingChat3();
	...
	methods:{
		...
		getremendianyingCount() {
			this.$http({
				url: `remendianying/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.remendianyingCount = data.data
				}
			})
		},
		remendianyingChat1() {
        ...
        var remendianyingChart1 = echarts.init(document.getElementById("remendianyingChart1"),'caravan');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                remendianyingChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    remendianyingChart1.resize();
                };
            }
        });
      })
    ...
    remendianyingChat2() {
      this.$nextTick(()=>{
        ...
        var remendianyingChart2 = echarts.init(document.getElementById("remendianyingChart2"),'caravan');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                remendianyingChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    remendianyingChart2.resize();
                };
            }
        });
      })
    ...
    remendianyingChat3() {
      this.$nextTick(()=>{
        ...
        var remendianyingChart3 = echarts.init(document.getElementById("remendianyingChart3"),'caravan');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                remendianyingChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    remendianyingChart3.resize();
                };
            }
        });
      })
        ...
        var dianyingxinxiChart1 = echarts.init(document.getElementById("dianyingxinxiChart1"),'caravan');
        this.$http({
            url: `dianyingxinxi/valueMul/title?yColumnNameMul=pingfen,pjs,wxs`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                var option = {};
                this.wordclouds(pArray,'dianyingxinxiChart2');
